...
enum ListsRepo {
    ...
    static func toggleLike(listID: Int) async throws {
        try await supa.rpc("toggle_list_like", params: ["p_list_id": listID]).execute()
    }
    ...
    static func addComment(listID: Int, body: String) async throws {
        struct Params: Encodable {
            let p_list_id: Int
            let p_body: String
        }
        try await supa.rpc("add_list_comment", params: Params(p_list_id: listID, p_body: body)).execute()
    }
